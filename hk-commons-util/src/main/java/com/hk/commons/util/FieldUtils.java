...
public abstract class FieldUtils {
	...
	public static Field findField(final Class<?> cls, String propertyName) {
		...
		Optional<Field> optional = fieldsList.stream().filter(item -> StringUtils.equals(item.getName(), propertyName)).findFirst();
		return optional.orElseThrow(() -> new IllegalArgumentException(cls.getName() + " no such propertyName :" + propertyName));
	}
