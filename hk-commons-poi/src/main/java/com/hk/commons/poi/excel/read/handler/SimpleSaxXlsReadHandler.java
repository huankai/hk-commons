...
public class SimpleSaxXlsReadHandler<T> extends AbstractSaxReadHandler<T> implements SaxXlsReadHandler<T>, HSSFListener {
	...
	public SimpleSaxXlsReadHandler(ReadableParam<T> param) {
		super(param);
	}
	 ...
	 * 
	 * @see
	 * org.apache.poi.hssf.eventusermodel.HSSFListener#processRecord(org.apache.poi.
	 * hssf.record.Record)
	 */
	@Override
	public void processRecord(Record record) {
		if (record.getSid() == BOFRecord.sid) { // 一个文件的开始,表示sheet或workbook的开始
			...
			if (br.getType() == BOFRecord.TYPE_WORKSHEET) { // 表示为 sheet
				isSheet = true;
		...
		if (currentSheetIndex == -1 || readParam.isParseAll() || (currentSheetIndex >= readParam.getSheetStartIndex()
			...
			if (null != value) {
				getRowColumnValues().add(new DefaultKeyValue<Integer, String>(thisColumn, value.toString()));
			}
				...
				if (rowNum >= readParam.getTitleRow()) { // 只有标题行及以下的行数据才解析
					if (isTitleRow(rowNum) && CollectionUtils.isEmpty(result.getTitleList())) { // 如果是标题行
						...
						try {
							...
							ErrorLog<T> errorLog = new ErrorLog<T>(sheetRecord.getSheetname(), rowNum,
