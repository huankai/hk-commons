...
public class ValueFormat {
	 ...
	 * 默认格式化方式
	 */
	private static Map<Object, DataFormat> DEFAULT_FROMAT;

	private Map<Object, DataFormat> mapper = new HashMap<Object, DataFormat>();

	static {
		DEFAULT_FROMAT = Maps.newHashMap();
	...
	public DataFormat getFormat(String propertyName, Class<?> propertyType) {
		DataFormat dataFormat = mapper.get(propertyName);
		if (null == dataFormat) {
			dataFormat = mapper.get(propertyType);
			if (null == dataFormat) {
				dataFormat = DataFormat.TEXT_FORMAT;
			}
		}
		return dataFormat;
