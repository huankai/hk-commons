 ...
 *
 */
public abstract class AbstractReadableExcel<T> implements ReadableExcel<T> {
	...
	public final ReadResult<T> read(File file) {
		try {
			ReadableHandler<T> readableHandler = createReadableHandler(file);
	...
	public ReadResult<T> read(InputStream in) {
		try {
			...
			ReadableHandler<T> readableHandler = createReadableHandler(in);
	 ...
	 */
	protected abstract ReadableHandler<T> createReadableHandler(InputStream in) throws IOException;
	 ...
	 * @return {@link ReadableHandler}
	 */
	protected abstract ReadableHandler<T> createReadableHandler(File file) throws IOException;
